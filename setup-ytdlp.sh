...
REPO_URL="https://github.com/tran-minhta/reclip.git"
TARGET_DIR="reclip"
REQUIRED_PACKAGES=(
  git
  curl
...
#done
apt update -y
for apt in "${REQUIRED_PACKAGES[@]}"; do
  if command -v "$apt" &>/dev/null || dpkg -l | grep -q -w "$apt"; then
    echo "✅ [$apt] đã có sẵn."
  else
    echo "⏳ Đang cài đặt [apt]..."
    apt install -y "$apt"
  fi
done
echo -e "\n=========================================="
